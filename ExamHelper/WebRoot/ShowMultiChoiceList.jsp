...
<script type="text/javascript" language="javascript">
	...
	var sectionName=document.getElementById("hiddenValue").value;
		...
		var keyword="<%=keywordFP%>"
			...
			{
				document.getElementById("fom").action = "${pageContext.request.contextPath}/multiChoice.do?flag=showMultiChoiceList&pageNow=1"
						 + "&sectionName=" + sectionName;
				document.getElementById("fom").submit();
			} else {

				document.getElementById("textfield").value = keyword;
				document.getElementById("fom").action = "${pageContext.request.contextPath}/multiChoice.do?flag=searchSubjectByKeyWord&pageNow=1"
				+"&sectionName=" + sectionName;
				document.getElementById("fom").submit();
			}
	}
											...
											<table style="width: 277px; ">
												<tr>
													<td width="538" style="width: 154px; height: 48px">题型选择：
														<select id="questionTypeChoose" onchange="chooseQuestionType();">
															<c:forEach items="${questionTypes}" var="type">
																<c:choose>
																<c:when test="${type.typeName==typeName}">
																<option id="${type.typeName}" selected="selected">${type.typeName}</option>
																</c:when>
																<c:otherwise>
													...
													<tr bgcolor="#FFFFFF">
														...
														<td><a
															href="${pageContext.request.contextPath}/multiChoice.do?flag=showMultiChoice&multiChoiceId=${multiChoice.id}">
																<c:set var="testStr" value="${multiChoice.questionStem}" />
																<c:choose>
